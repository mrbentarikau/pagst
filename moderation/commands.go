...
var ModerationCommands = []*commands.YAGCommand{
	{
		...
		Description:   "Bans a user, specify number of days of messages to delete with -ddays (0 to 7)",
		RequiredArgs:  1,
	...
	},
	{
		...
		Description:   "Removes the specified role from the member",
	...
	return func(p *paginatedmessages.PaginatedMessage, page int) (interface{}, error) {
		...
		if len(result) > 0 {
			...
			for _, entry := range result {
				...
				entry_formatted += "\n"
				purgedWarnLogs := logs.ConfRestrictLogsThirtyDays.GetBool() && entry.CreatedAt.Before(time.Now().AddDate(0, 0, -30))
