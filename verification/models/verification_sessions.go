...
var VerificationSessionTableColumns = struct {
	Token     string
	UserID    string
	GuildID   string
	CreatedAt string
	...
	Token:     "verification_sessions.token",
	UserID:    "verification_sessions.user_id",
	GuildID:   "verification_sessions.guild_id",
	CreatedAt: "verification_sessions.created_at",
	SolvedAt:  "verification_sessions.solved_at",
	ExpiredAt: "verification_sessions.expired_at",
}
...
var (
	...
	verificationSessionColumnsWithoutDefault = []string{"token", "user_id", "guild_id", "created_at"}
	verificationSessionColumnsWithDefault    = []string{"solved_at", "expired_at"}
	verificationSessionPrimaryKeyColumns     = []string{"token"}
	verificationSessionGeneratedColumns      = []string{}
)

type (
	// VerificationSessionSlice is an alias for a slice of pointers to VerificationSession.
	// This should almost always be used instead of []VerificationSession.
...
// CountG returns the count of all VerificationSession records in the query using the global executor
...
func VerificationSessions(mods ...qm.QueryMod) verificationSessionQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"verification_sessions\".*"})
	}

	return verificationSessionQuery{q}
}
