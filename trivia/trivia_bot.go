...
func (t *triviaSession) buildEmbed() *discordgo.MessageEmbed {
	...
	lenOptions := len(t.Question.Options)
	for i, v := range t.Question.Options {
		optionFieldContent := fmt.Sprintf("\n%s %s\n", t.optionEmojis[i], v)
		if t.ended && v != t.Question.Answer && i < lenOptions-1 {
			optionsField.Value += fmt.Sprintf("~~%s~~", optionFieldContent)
		} else if t.ended && v != t.Question.Answer {
			optionsField.Value += fmt.Sprintf("~~%s\n~~", optionFieldContent)
		} else if i < lenOptions-1 {
			optionsField.Value += fmt.Sprintf("** %s ** ", optionFieldContent)
		} else {
			optionsField.Value += fmt.Sprintf("** %s\n ** ", optionFieldContent)
		}
...
func (t *triviaSession) handleInteractionAdd(evt *eventsystem.EventData) {
	...
	// Editing the embed can sometime get rate-limited
