...
var JoinedGuildTableColumns = struct {
	ID          string
	JoinedAt    string
	LeftAt      string
	MemberCount string
	...
	ID:          "joined_guilds.id",
	JoinedAt:    "joined_guilds.joined_at",
	LeftAt:      "joined_guilds.left_at",
	MemberCount: "joined_guilds.member_count",
	Name:        "joined_guilds.name",
	OwnerID:     "joined_guilds.owner_id",
	Avatar:      "joined_guilds.avatar",
}
...
var (
	...
	joinedGuildColumnsWithoutDefault = []string{"id", "joined_at", "member_count", "name", "owner_id", "avatar"}
	joinedGuildColumnsWithDefault    = []string{"left_at"}
	joinedGuildPrimaryKeyColumns     = []string{"id"}
	joinedGuildGeneratedColumns      = []string{}
)

type (
	// JoinedGuildSlice is an alias for a slice of pointers to JoinedGuild.
	// This should almost always be used instead of []JoinedGuild.
...
// CountG returns the count of all JoinedGuild records in the query using the global executor
...
func JoinedGuilds(mods ...qm.QueryMod) joinedGuildQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"joined_guilds\".*"})
	}

	return joinedGuildQuery{q}
}
