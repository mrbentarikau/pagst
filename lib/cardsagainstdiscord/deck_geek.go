...
func init() {
	pack := &CardPack{
		...
		Prompts: []*PromptCard{
			{Prompt: `%s is OP. Please nerf.`},
			{Prompt: `%s is way better in %s mode. (Pax East 2014)`},
			{Prompt: `%s: Achievement unlocked. (Pax Prime 2013)`},
			{Prompt: `(Heavy breathing) Luke, I am %s. (Pax East 2014)`},
			{Prompt: `Press ↓ ↓ ← → B to unleash %s. (Pax East 2013 Promo Pack C)`},
			{Prompt: `What made Spock cry? (Pax Prime 2013)`},
			{Prompt: `What's the latest bullshit that's troubling this quaint fantasy town? (Pax Prime 2013)`},
		},
