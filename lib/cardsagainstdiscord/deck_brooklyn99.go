...
func init() {
	pack := &CardPack{
		...
		Prompts: []*PromptCard{
			{Prompt: `That's how we do %s in the Nine-Nine.`},
			{Prompt: `Peralta, what are you doing? %s `},
			{Prompt: `I once got arrested for %s `},
			{Prompt: `The worst thing to say to a cop is %s`},
			{Prompt: `The worst thing do in front of a cop is %s`},
			{Prompt: `The best undercover name is %s`},
			{Prompt: `The most disturbing thing I've seen as a cop is%s`},
			{Prompt: `The most bad ass catchprase is %s`},
			{Prompt: `Someone painted %s on our minivan`},
			{Prompt: `Terry loves %s`},
			{Prompt: `Eyes up here, Gina. I'm more than %s`},
			{Prompt: `19 milk less years I have gone, and for some reason I can't quit %s?`},
			{Prompt: `%s, from %s. Sounds like a fun treat.`},
			{Prompt: `Oh, I'm as serious as %s`},
			{Prompt: `Amy Santiago was actually late to work because of %s`},
			{Prompt: `When will you quit police work and pursue your dream of %s?`},
			{Prompt: `I'm making a scrapbook as a present for Capt. Holt. It's titled %s`},
			{Prompt: `They do call me Tenacious Ray down at the country club. Because for the past 10 years, I've been suing them for %s`},
			{Prompt: `The most intimate thing you can do to your lover is %s, other than washing their hair`},
			{Prompt: `No, from now on call me, %s.`},
			{Prompt: `Ew, it's like dating %s.`},
			{Prompt: `I know, it's so hot. %s`},
			{Prompt: `These silences are very hard for me to read. What's the vibe in here? %s`},
			{Prompt: `We're %s, Charles!`},
			{Prompt: `The alcohol has rendered me %s.`},
			{Prompt: `What is it about me that screams loser? %s`},
			{Prompt: `There's nothing more intoxicating than %s`},
			{Prompt: `I've been asked to deliver a message of hope. This is what I have so far: %s.`},
			{Prompt: `The reason for Jake's financial issues is %s`},
			{Prompt: `Is that Mr. MacLaine I hear? Cause someone's about to %s`},
			{Prompt: `Seriously, what happened to Hitchcock and Scully? %s`},
			{Prompt: `The hardest case I've ever solved involved %s`},
			{Prompt: `My plan to win the Halloween Heist is simple. It will involve %s`},
			{Prompt: `Who is the real Amazing Detective/Human/Genius? %s`},
			{Prompt: `The only thing worse than failing to catch a criminal is %s`},
			{Prompt: `The night shift is keeping you and Kevin apart. You two just need to %s.`},
			{Prompt: `Captain banned %s from the office due to the Gina incident.`},
			{Prompt: `She sabotaged my career because I refused to %s.`},
			{Prompt: `Dig deep, think of something scary %s`},
			{Prompt: `I referred to myself as %s to several beautiful Latina ladies.`},
			{Prompt: `The Nine-Nine hates the Fire Department because of %s`},
			{Prompt: `Here are your drinks, and %s from Mr. Boyle`},
			{Prompt: `Good point. Rosa would want a much bigger %s.`},
			{Prompt: `Here at the Fun Zone, we live by one rule. When it's your birthday you're always %s.`},
			{Prompt: `Gina Linetti has a new dance troupe called %s.`},
			{Prompt: `%s are dumb.`},
			{Prompt: `My kids think their pre-school teacher's %s.`},
			{Prompt: `Tiny Terry loves his %s.`},
			{Prompt: `I need you to unleash %s`},
			{Prompt: `I can't hear you Kevin, you're %s.`},
			{Prompt: `Capt. Holt should name his car %s instead of Gertie.`},
			{Prompt: `Pimento has %s disease.`},
			{Prompt: `- I don't want to say what using good vocabulary gets me from Amy: %s.`},
			{Prompt: `Get familiar with %s or get familiar with the unemployment line.`},
			{Prompt: `No, I don't wanna listen Boyle. You are trying to %s and that will not fly!`},
			{Prompt: `What does the Ebony Falcon do? %s`},
			{Prompt: `You've lost the ability to %s. You just plain boring.`},
			{Prompt: `The night that you %s for 20 seconds and I became obsessed with you forever`},
			{Prompt: `I'll %s to perk up my little man`},
			{Prompt: `My dreams are coming true. You and me getting %s together.`},
			{Prompt: `Up is up. Down is down. The Vulture still %s.`},
			{Prompt: `What's your happy place? %s`},
			{Prompt: `Say %s again and I will shoot you in the stomach.`},
			{Prompt: `He says %s just like you.`},
			{Prompt: `You call that %s?`},
			{Prompt: `Fine, but we didn't have to endure this crap when we danced in %s.`},
			{Prompt: `Your dress makes you look like %s.`},
			{Prompt: `Damn it, %s! You sold me out.`},
			{Prompt: `%s got served.`},
			{Prompt: `You embarrassed yourself in front of %s.`},
			{Prompt: `The ghost of the %s haunts the precinct!`},
			{Prompt: `She had a replacement hip with some serious %s`},
			{Prompt: `It's like %s with a transformer`},
			{Prompt: `Kind, sober and %s`},
			{Prompt: `Time for me to get out there and %s`},
			{Prompt: `It's not your fault, I was %s`},
			{Prompt: `It was slightly less %s with you.`},
			{Prompt: `Everytime you shake me, I see it %s.`},
			{Prompt: `My mother has a fantastic %s.`},
			{Prompt: `I %s this stewardess in Sweden once.`},
			{Prompt: `And sir, nothing would make me %s than being your big daddy.`},
			{Prompt: `Something no lover of yours has ever %s.`},
			{Prompt: `I got a nice soft %s so I don't intimidate the husbands.`},
			{Prompt: `Oh honey, that flat ass is perched right on top of my %s.`},
			{Prompt: `Because I was your %s and your lover? Sorry I just can't control my %s around you.`},
			{Prompt: `The only way out of this hole is to %s.`},
			{Prompt: `I can picture you as a teenager just %s all over New York City.`},
			{Prompt: `No hole's too tight for these %s tips.`},
			{Prompt: `There's %s in our genes.`},
			{Prompt: `I just got it out of the vent to rub it in your %s`},
			{Prompt: `Don't be shy, just say what you will do international sex symbol %s's body.`},
			{Prompt: `Bill is %s. He's able to use more than just his mouth.`},
			{Prompt: `There's not even any soft parts in the middle we can %s`},
			{Prompt: `Everybody stop %s! We just had harassment training.`},
			{Prompt: `There are no %s in Scottdale, Margo.`},
			{Prompt: `Changed my relationship status to %s. Pony up y'all!`},
			{Prompt: `I came down with a big old %s infection.`},
			{Prompt: `Ain't nothing but %s.`},
			{Prompt: `Teddy is %s people out of the competition.`},
			{Prompt: `You've been in there like five times. What are you gonna do, annoy him into %s?`},
			{Prompt: `Do you need a %s tutor because the department will provide you one.`},
			{Prompt: `Dad said you were giving us tickets to the %s movie.`},
			{Prompt: `Sorry sir that no one here wants to %s, you dusty old %s.`},
			{Prompt: `This is a direct order. Stop %s.`},
			{Prompt: `Looks like a sticky %s.`},
			{Prompt: `When I was alone in her office, I changed her autocorrect. When she types in Wuntch, it gets replaced with %s.`},
			{Prompt: `Where's Jake? Did you %s him, Terry?`},
			{Prompt: `Thank you for %s my feelings.`},
			{Prompt: `The magic of %s. I'm in a state of total euphoria.`},
			{Prompt: `Hey-hey! New %s alert! `},
		},
