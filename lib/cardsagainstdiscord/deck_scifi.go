...
func init() {
	pack := &CardPack{
		...
		Prompts: []*PromptCard{
			{Prompt: `Computer! Display %s on screen. Enhance.`},
			{Prompt: `Fear leads to anger. Anger leads to hate. Hate leads to %s.`},
			{Prompt: `Madam President, the asteroid is headed directly for Earth and there's only one thing that can stop it: %s.`},
			{Prompt: `This won't be like negotiating with the Vogons. Humans only respond to one thing: %s.`},
			{Prompt: `What is the answer to life, the universe, and everything?`},
			{Prompt: `You have violated the Prime Directive! You exposed an alien culture to %s before they were ready.`},
			{Prompt: `You're not going to believe this, but I'm you from the future! You've got to stop %s.`},
		},
